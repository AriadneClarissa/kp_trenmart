...
<div class="container py-5">
    ...
    <form action="<?php echo e(route('admin.judul.update')); ?>" method="POST" id="mainForm">
        <?php echo csrf_field(); ?>
            ...
            <div class="card-body p-4">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="judul_terbaru" class="form-control rounded-pill px-4 shadow-sm border-0 bg-light" value="<?php echo e($judulTerbaru); ?>" placeholder="Contoh: Produk Terbaru">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-semibold text-muted small">Judul 2</label>
                        <input type="text" name="judul_terpopuler" class="form-control rounded-pill px-4 shadow-sm border-0 bg-light" value="<?php echo e($judulTerpopuler); ?>" placeholder="Contoh: Produk Terpopuler">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card shadow-sm border-0 rounded-4 mb-4">
            <div class="card-body p-4">
                ...
                <select name="target_section" id="dropdownTargetSection" class="form-select rounded-pill px-4 shadow-sm border-0 bg-light">
                    <?php $targetOld = old('target_section', $selectedSection ?? 'section_3'); ?>
                    <option value="section_3" <?php echo e($targetOld == 'section_3' ? 'selected' : ''); ?>>Section 3 (Custom)</option>
                    <option value="terpopuler" <?php echo e($targetOld == 'terpopuler' ? 'selected' : ''); ?>>Section Terpopuler</option>
                    <option value="terbaru" <?php echo e($targetOld == 'terbaru' ? 'selected' : ''); ?>>Section Terbaru</option>
                </select>
            </div>
        </div>
            ...
            <div class="card-body p-4">
                ...
                <div class="mt-4">
                    ...
                    <div id="listProdukTerpilih" class="d-flex flex-wrap gap-2">
                        ...
                        <?php $__currentLoopData = $produkTerpilih; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="badge bg-primary rounded-pill px-3 py-2 d-flex align-items-center gap-2 item-tag" data-id="<?php echo e($item->kd_produk); ?>">
                                <span><?php echo e($item->nama_produk); ?></span>
                                <input type="hidden" name="produk_pilihan[]" value="<?php echo e($item->kd_produk); ?>">
...
$(document).ready(function() {
    const targetOld = "<?php echo e(old('target_section', $selectedSection ?? 'section_3')); ?>";
    const sectionProducts = <?php echo json_encode(collect($sectionProdukMap)->map(fn ($items) => $items->map(fn ($item) => [
        'id' => $item->kd_produk, 'name' => $item->nama_produk, ])->values())->toArray()) ?>;
    const initialSelectedProducts = <?php echo json_encode($produkTerpilih->map(fn ($item) => [
        'id' => $item->kd_produk, 'name' => $item->nama_produk, ])->values()) ?>;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Lenovo LOQ\Documents\GitHub\kp_trenmart\sistemPenjualanTrenmart\resources\views/admin/edit_judul.blade.php ENDPATH**/ ?>